...
required_packages <- c(
  "shiny", "shinyjs", "readr", "randomForest", "furrr", "future", 
  "dplyr", "tools", "stringr", "purrr", "here"
)
...
predict_single_sample <- function(sample_df, models, class_levels, accuracy_thresholds) {
  expected_features <- get_expected_features(models[[1]])
  missing_cols <- setdiff(expected_features, colnames(sample_df))
  if (length(missing_cols) > 0) sample_df[missing_cols] <- 0
  sample_df <- sample_df[, expected_features, drop = FALSE]
  
  prob_vec <- sapply(class_levels, function(class_i) {
    mod <- models[[class_i]]
    predict(mod, sample_df, type = "prob")[, "Yes"]
  })
  prob_vec <- setNames(as.numeric(prob_vec), class_levels)
  
  pred_class <- names(prob_vec)[which.max(prob_vec)]
  pred_prob <- max(prob_vec)
  threshold <- accuracy_thresholds[pred_class]
  is_accurate <- !is.na(threshold) && pred_prob >= threshold
  
  notification <- if (is.na(threshold)) {
    paste0("No threshold for class '", pred_class, "'.")
  } else if (is_accurate) {
    paste0("Prediction is ABOVE threshold (", threshold, ").")
  } else {
    paste0("Prediction is BELOW threshold (", threshold, ").")
  }
  
  tibble(
    Predicted_Class = pred_class,
    Predicted_Probability = round(pred_prob, 4),
    Above_Threshold = ifelse(is_accurate, "Yes", "No"),
    ...
    withProgress(message = "Running prediction...", value = 0, {
      ...
      if (n_samples < 20) {
        ...
        for (i in seq_len(n_samples)) {
          ...
          pred <- predict_single_sample(row_data, models, class_levels, accuracy_thresholds)
        ...
        for (i in seq_along(sample_batches)) {
          ...
          batch_results <- future_map_dfr(
            ...
            .options = furrr_options(packages = c("randomForest"))
          )
